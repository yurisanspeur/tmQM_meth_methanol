...
def place_methanol(M_O_bondlength, mol, charge):
    ...
    M_index = np.where(np.array([x.is_metal for x in complex_.atoms]))[0][0]
    # Center the complex with the metal center at the origin

    complex_.coordinates = complex_.coordinates - complex_.atoms[M_index].coord


    # Get the location of the halogen that we want to pop out #FIXME Make all the indexing dynamic and robust
    halide_atoms_indices = [ i for (i, atom) in enumerate(complex_.atoms) if atom.label in ['F', 'Cl', 'Br', 'I', 'At'] ]
    ...
    halide_location = complex_.atoms.pop(pop_index).coord # TODO: Get the index from Nick's work
    ...
    M_index = np.where(np.array([x.is_metal for x in complex_.atoms]))[0][0] # Account for re-index
    vec = methanol_final.atoms[0].coord - complex_.atoms[M_index].coord
    M_O_distance = np.linalg.norm(vec)
    # Let's target an M-O distance of 2.5 Ang
    distance_factor = M_O_bondlength / M_O_distance
    target_location = complex_.atoms[M_index].coord + distance_factor * vec
    ...
    methanol_final.atoms[2].coord = [methanol_final.atoms[0].coord[0] + bondlength * np.sin(angle), y_pos , methanol_final.atoms[0].coord[2] + bondlength * np.cos(angle)]
    ...
    counter = 0
    while COH_angle > target_angle_COH and abs(COH_angle - target_angle_COH) > 0.01 and counter < 5000:
        angle -= 0.001
        print(angle, "angle")
        methanol_final.atoms[2].coord = [methanol_final.atoms[0].coord[0] + bondlength * np.sin(angle), y_pos , methanol_final.atoms[0].coord[2] + bondlength * np.cos(angle)]
        COH_angle = complex_.angle(0, O_index, C_index).to(units="deg")
    ...
    #O_index -= 1
    MOH_angle = complex_.angle(M_index, O_index, H_index).to(units="deg")
    ...
    atoms_to_xyz_file(complex_.atoms, f"backbone_{mol.split('.')[0]}.xyz")
    ...
    print_combined_molecule(core_xyz_filename=f"backbone_{mol.split('.')[0]}.xyz",
                            atoms_to_del=atoms_to_del,
                            frag_names=['Me'],
                            name=f"target_intermediate_{mol.split('.')[0]}")
...
if __name__ == '__main__':
    molecules = open('selected_halides.txt').readlines()
    for mol in molecules: 
        bondlength = 2.5 # Initial bondlength
        mol = mol.split('\n')[0]
        charge = 0
        try:
            place_methanol(bondlength, mol, charge)
        except:
            print(f"Failed for molecule {mol}")

    # Call the place_methanol function
...
#            place_methanol(bondlength, mol, charge)
#        except RuntimeError:
#            # Call the function with the previous bondlength which did not trigger the Exception
#            print("Too close to metal center. Using the previous bondlengh and exiting...")
#            bondlength += 0.1
#            place_methanol(bondlength, mol, charge)
#            break
#
